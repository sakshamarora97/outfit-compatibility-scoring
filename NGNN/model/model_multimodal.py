...
import numpy as np
#TODO from tensorflow.contrib.rnn import GRUCell
...
def weights(name, hidden_size, i):
    ...
    if name == 'in_image':
        ...
        #w = tf.compat.v1.get_variable(name='gnn/w/in_image_', shape=[2048, hidden_size], initializer=tf.random_normal_initializer)
    if name == 'out_image':
        w = tf.compat.v1.get_variable(name='w/out_image_' + str(i),
                            shape=[hidden_size, 2048],
                            initializer=tf.random_normal_initializer(stddev=image_stdv))
        #w = tf.compat.v1.get_variable(name='w/out_image_', shape=[hidden_size, 2048], initializer=tf.random_normal_initializer)
    if name == 'in_text':
        w = tf.compat.v1.get_variable(name='w/in_text_'+ str(i),
                            shape=[2757, hidden_size],
                            initializer=tf.random_normal_initializer(stddev=image_stdv))
        #w = tf.compat.v1.get_variable(name='gnn/w/in_image_', shape=[2048, hidden_size], initializer=tf.random_normal_initializer)
    if name == 'out_text':
        w = tf.compat.v1.get_variable(name='w/out_text_' + str(i),
    ...
    if name == 'image_hidden_state_out':
        ...
        #w = tf.compat.v1.get_variable(name='w/hidden_state_out_' + str(i), shape=[hidden_size, hidden_size], initializer=tf.random_normal_initializer)
    if name == 'image_hidden_state_in':
        #w = tf.compat.v1.get_variable(name='w/hidden_state_in_', shape=[hidden_size, hidden_size], initializer=tf.random_normal_initializer)
        w = tf.compat.v1.get_variable(name='w/image_hidden_state_in_' + str(i),
                            shape=[hidden_size, hidden_size],
                            initializer=tf.random_normal_initializer(stddev=hidden_stdv))
    if name == 'text_hidden_state_out':
        w = tf.compat.v1.get_variable(name='w/text_hidden_state_out' + str(i),
                            shape=[hidden_size, hidden_size],
                            initializer=tf.random_normal_initializer(stddev=hidden_stdv))
        #w = tf.compat.v1.get_variable(name='w/hidden_state_out_' + str(i), shape=[hidden_size, hidden_size], initializer=tf.random_normal_initializer)
    if name == 'text_hidden_state_in':
        #w = tf.compat.v1.get_variable(name='w/hidden_state_in_', shape=[hidden_size, hidden_size], initializer=tf.random_normal_initializer)
        w = tf.compat.v1.get_variable(name='w/text_hidden_state_in_' + str(i),
                            shape=[hidden_size, hidden_size],
                            initializer=tf.random_normal_initializer(stddev=hidden_stdv))
...
def biases(name, hidden_size, i):
    ...
    hidden_stdv = np.sqrt(1. / (hidden_size))
    if name == 'image_hidden_state_out':
        b = tf.compat.v1.get_variable(name='b/image_hidden_state_out' + str(i), shape=[hidden_size],
                        initializer=tf.random_normal_initializer(stddev=hidden_stdv))
        # b = tf.compat.v1.get_variable(name='b/hidden_state_out', shape=[hidden_size],
        #                 initializer=tf.random_normal_initializer)
    if name == 'image_hidden_state_in':
        b = tf.compat.v1.get_variable(name='b/image_hidden_state_in' + str(i), shape=[hidden_size],
                        initializer=tf.random_normal_initializer(stddev=hidden_stdv))
        # b = tf.compat.v1.get_variable(name='b/hidden_state_in', shape=[hidden_size],
        #                 initializer=tf.random_normal_initializer)
    if name == 'text_hidden_state_out':
        b = tf.compat.v1.get_variable(name='b/text_hidden_state_out' + str(i), shape=[hidden_size],
                        initializer=tf.random_normal_initializer(stddev=hidden_stdv))
        # b = tf.compat.v1.get_variable(name='b/hidden_state_out', shape=[hidden_size],
        #                 initializer=tf.random_normal_initializer)
    if name == 'text_hidden_state_in':
        b = tf.compat.v1.get_variable(name='b/text_hidden_state_in' + str(i), shape=[hidden_size],
                        initializer=tf.random_normal_initializer(stddev=hidden_stdv))
        # b = tf.compat.v1.get_variable(name='b/hidden_state_in', shape=[hidden_size],
        #                 initializer=tf.random_normal_initializer)
    if name == 'out_image':
        # b = tf.compat.v1.get_variable(name='b/out_image_', shape=[2048],
        #                     initializer=tf.random_normal_initializer)
        b = tf.compat.v1.get_variable(name='b/out_image_' + str(i), shape=[2048],
                            initializer=tf.random_normal_initializer(stddev=image_stdv))
    if name == 'out_text':
        # b = tf.compat.v1.get_variable(name='b/out_image_', shape=[2048],
        #                     initializer=tf.random_normal_initializer)
        b = tf.compat.v1.get_variable(name='b/out_text_' + str(i), shape=[2757],
                            initializer=tf.random_normal_initializer(stddev=image_stdv))
