...
#TODOfrom tensorflow.contrib.rnn import GRUCell
import os
##################load data###################
...
def cm_ggnn(batch_size, image_hidden_size, text_hidden_size, n_steps, learning_rate, G, num_category, opt, i, beta):
    ...
    if i == 0:
        with tf.compat.v1.variable_scope("cm_ggnn", reuse=None):
            w_conf_image = tf.compat.v1.get_variable(name='gnn/w/conf_image', shape=[image_hidden_size, 1],
                                           initializer=tf.random_normal_initializer(hidden_stdv))
            w_score_image = tf.compat.v1.get_variable(name='gnn/w/score_image', shape=[image_hidden_size, 1],
                                            initializer=tf.random_normal_initializer(hidden_stdv))
            w_conf_text = tf.compat.v1.get_variable(name='gnn/w/conf_text', shape=[text_hidden_size, 1],
                                           initializer=tf.random_normal_initializer(hidden_stdv))
            w_score_text = tf.compat.v1.get_variable(name='gnn/w/score_text', shape=[text_hidden_size, 1],
                                            initializer=tf.random_normal_initializer(hidden_stdv))
            #w_atten = tf.compat.v1.get_variable(name='gnn/w/atten', shape=[num_category, num_category], initializer=tf.random_normal_initializer(num_stdv))
    else:
        with tf.compat.v1.variable_scope("cm_ggnn"):
    ...
    text_pos = tf.compat.v1.placeholder(tf.float32, [batch_size, num_category, 2757])
    text_neg = tf.compat.v1.placeholder(tf.float32, [batch_size, num_category, 2757])
    graph_pos = tf.compat.v1.placeholder(tf.float32, [batch_size, num_category, num_category])
    graph_neg = tf.compat.v1.placeholder(tf.float32, [batch_size, num_category, num_category])

    ##################GGNN's output###################
    with tf.compat.v1.variable_scope("gnn_image", reuse=None):
    ...
    with tf.compat.v1.variable_scope("gnn_text", reuse=None):
    ...
    for variable in tf.compat.v1.trainable_variables():
        print (variable)
        cost_parameter += tf.compat.v1.keras.regularizers.l2(0.1)(variable)
        num_parameter += 1.
...
#        tf.compat.v1.disable_v2_behavior()
        ...
        train_size, train_size_ = load_train_size() ##16,983*8 = 1,35,864 --> ##1000(outfits)*8(per_outfit) - 8,000 Items
        print ('train_size is %d' % train_size_) 
        train_batch = int(train_size_ / batch_size)
        print ('train_batch is %d' % train_batch) ## 16*(8492) = 1,35,864 --> ##16*(500) = 8,000

#        for epoch in range(30): 
        for epoch in range(4):#ADDED TODO
            #########train##########
            test_interval = 22 #TODO EDITED 2000
            if epoch > 10:
                test_interval = 22 #TODO EDITED 1000
            print(f"Epoch {epoch}")
            no_count = 0
            c_all = 0.
            score_all = 0.
            ...
            for i in range(train_batch):
                if i>45: ##ADDED TODO ##45 batches only of size batch_size 45*32=1440 
                     break
                print(f"Number {i}")
                ...
                print(f"size_:{size_},batch_size:{batch_size}")
                if size_ >= batch_size:
                    ...
                    test_size_fitb = load_test_size() ##TODO ADDed
                    test_size_auc = load_test_size() ##TODO ADDed
                ## TODO Commented
                    if i % test_interval == 0:
                        print(f"would have predicted test here: at i = {i}")
                        pass
            ...
            for i in range(batches):
                ...
                for j in range(int(batch_size / 2)): #ADDED TODO
                    a = []
                    for k in range(j * 2, (j + 1) * 2):
                        a.append(answer[k][0])
                    if np.argmax(a) == 0:
...
if __name__ == '__main__':
    ...
    best_accurancy = 0.
    i = 0
#    batch_size = 16 TODO ADDED CONFIG
    batch_size = 32 
